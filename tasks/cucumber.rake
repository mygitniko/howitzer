...
unless ARGV.any? {|a| a =~ /^gems/} # Don't load anything when running the gems:* tasks
  begin
    ...
    namespace :cucumber do
      Cucumber::Rake::Task.new({phantomjs: 'db:test:prepare'}, 'Run features that marked phantomjs') do |t|
        t.fork = false
        t.profile = 'phantomjs'
      end

      Cucumber::Rake::Task.new({poltergeist: 'db:test:prepare'}, 'Run features that marked poltergeist') do |t|
        t.fork = false
        t.profile = 'poltergeist'
      end
