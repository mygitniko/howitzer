...
Transform /CUSTOMER_FEEDBACK_DATA(\d*)(?:\[\:(.+)\])?/ do |num, property|
  res = Gen::given_customer_data_by_number(num)
  res = res.send(property) if property
  res
end
...
Transform /^table:.*$/ do |table|
  raw = table.raw.map do |array|
    array.map do |el|
      ...
      data = /CUSTOMER_FEEDBACK_DATA(?<num>\d*)(?:\[\:(?<property>.+)\])?/.match(el)
      if data
        res = Gen::given_customer_data_by_number(data[:num])
        if data[:property]
          res = res.send(data[:property])
        end
      end
